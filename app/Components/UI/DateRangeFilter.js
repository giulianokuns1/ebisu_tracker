...
export default function DateRangeFilter({ value, onChange, onClear }) {
    ...
    return <div className={styles.container}>
        ...
        {open && <div className={styles.popover} role="dialog" aria-label="Date range filter"><div className={styles.presets}><button type="button" onClick={() => applyPreset(thisMonth, today)}>This Month</button><button type="button" onClick={() => applyPreset(lastThirty, today)}>Last 30 Days</button><button type="button" onClick={() => applyPreset(thisYear, today)}>This Year</button></div><DatePicker inline selectsRange startDate={draft[0]} endDate={draft[1]} onChange={selectRange} /><div className={styles.footer}><span>{draft[0] && !draft[1] ? 'Select an end date' : 'Range applied automatically'}</span><button type="button" onClick={() => { setDraft([null, null]); onClear(); setOpen(false); }}>Clear</button></div></div>}
    </div>;
}
