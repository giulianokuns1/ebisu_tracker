...
export default function SavingsDashboard({ data, onRefresh, range }) {
    ...
    return <div className={styles.dashboard}>
        <section className={styles.metrics}><Metric title={t('Total Savings Balance')} values={totals.map((item) => `${item.symbol || ''} ${format(item.balance)}`)} tone="teal" icon="bi-wallet2" /><Metric title={t('Monthly Saved')} values={totals.map((item) => `${item.symbol || ''} ${format(item.monthly)}`)} tone="blue" icon="bi-cash-stack" /><Metric title={t('Savings Goals')} values={[`${goals.length} ${t('active')}`]} tone="violet" icon="bi-piggy-bank" /><Metric title={t('Avg. Goal Progress')} values={[`${goals.length ? (goals.reduce((sum, goal) => sum + goal.percentage, 0) / goals.length).toFixed(0) : 0}%`]} tone="orange" icon="bi-bar-chart-line" /></section>
        <section className={styles.grid}><article className={styles.panel}><div className={styles.chartHeader}><h2>{chartMode === 'balance' ? t('Savings Balance Overview') : t('Net Savings Activity')}</h2><div className={styles.chartControls}><div className={styles.chartMode} aria-label={t('Chart type')}><button type="button" className={chartMode === 'balance' ? styles.activeChartMode : ''} onClick={() => setChartMode('balance')}>{t('Balance')}</button><button type="button" className={chartMode === 'activity' ? styles.activeChartMode : ''} onClick={() => setChartMode('activity')}>{t('Activity')}</button></div>{chartCurrencies.length > 1 && <select value={selectedCurrencyId} onChange={(event) => setChartCurrencyId(event.target.value)} aria-label={t('Chart currency')}>{chartCurrencies.map((currency) => <option key={currency.id} value={currency.id}>{currency.name} ({currency.symbol})</option>)}</select>}</div></div><div className={styles.chart}>{chartMode === 'balance' ? <Line data={chartData} options={chartOptions} /> : <Bar data={chartData} options={chartOptions} />}</div></article><article className={styles.panel}><h2>{t('Savings Goals')}</h2><div className={styles.goals}>{goals.map((goal, index) => <Link href={`/savings/details/${goal.id}`} className={styles.goal} key={goal.id}><div><strong>{goal.name}</strong><span>{goal.currency_symbol} {format(goal.balance)} / {goal.currency_symbol} {format(goal.target_amount)}</span><div className={styles.progress}><i style={{ width: `${goal.percentage}%`, background: colors[index % colors.length] }} /></div></div><b>{goal.percentage.toFixed(0)}%</b></Link>)}</div><Link href="/savings/create" className={styles.addGoal}><i className="bi bi-plus-lg" /> {t('Add New Goal')}</Link></article></section>
        ...
        <Dialog visible={open} onHide={() => setOpen(false)} header={editingId ? t('Edit Saving') : t('Add Saving')} className={styles.dialog}><div className={styles.transactionForm}><label>{t('Goal')}<select value={form.savingId} onChange={(event) => setForm({ ...form, savingId: event.target.value })}><option value="">{t('Unassigned Savings')}</option>{goals.map((goal) => <option key={goal.id} value={goal.id}>{goal.name}</option>)}</select></label><label>{t('Currency')}<select value={form.currencyId} onChange={(event) => setForm({ ...form, currencyId: event.target.value })}>{data.currencies?.map((currency) => <option key={currency.id} value={currency.id}>{currency.name} {currency.symbol}</option>)}</select></label><label>{t('Amount')}<input type="number" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></label><label>{t('Date')}<input type="date" value={form.transactionDate} onChange={(event) => setForm({ ...form, transactionDate: event.target.value })} /></label><label className={styles.full}>{t('Comment')}<input value={form.comment} onChange={(event) => setForm({ ...form, comment: event.target.value })} /></label>{editingId && <button className={styles.removeSaving} type="button" onClick={() => setConfirmOpen(true)}>{t('Remove Saving')}</button>}<button className={styles.saveSaving} type="button" onClick={save}>{editingId ? t('Update Saving') : t('Save Saving')}</button></div></Dialog>
        <Dialog visible={confirmOpen} onHide={() => setConfirmOpen(false)} header={t('Remove saving')} className={styles.dialog}><div className={styles.removeConfirmation}><p>{t('This action permanently removes this saving entry.')}</p><div className={styles.confirmActions}><button type="button" onClick={() => setConfirmOpen(false)}>{t('Cancel')}</button><button className={styles.removeSaving} type="button" onClick={remove}>{t('Remove Saving')}</button></div></div></Dialog>
    </div>;
}
