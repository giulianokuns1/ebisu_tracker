...
export default function SavingsForm({ savingId }) {
    ...
    return <div className={styles.goalDetailLayout}><FormShell><form className={styles.goalForm} onSubmit={save}><FormInput label="Goal Name" type="text" value={values.name} onChange={update('name')} /><FormInput label="Target Amount" type="number" value={values.targetAmount} onChange={update('targetAmount')} /><FormSelect label="Currency" values={currencies} valueLabel="name" multipleValueLabel={['name', 'symbol']} value={values.currencyId} onChange={update('currencyId')} defaultLabel="Select a Currency" /><FormInput label="Starting Amount" type="number" value={values.startingAmount} onChange={update('startingAmount')} /><div className={styles.commentField}><label>Comment</label><textarea value={values.comment} onChange={update('comment')} placeholder="Optional note about this goal" /></div><FormActionBar editing={Boolean(savingId)} onCancel={() => router.push('/savings')} onDelete={remove} createLabel="Create Goal" updateLabel="Update Goal" /></form></FormShell>{savingId && <aside className={styles.goalTransactions}><h3>Goal activity</h3>{transactions.length ? transactions.map((transaction) => <div key={transaction.id}><span>{new Date(transaction.transaction_date).toLocaleDateString()}</span><p>{transaction.comment || 'Saved money'}</p><b>{transaction.currency_symbol} {transaction.amount}</b></div>) : <p>No savings added to this goal yet.</p>}</aside>}</div>;
}
