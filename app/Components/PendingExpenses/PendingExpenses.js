...
const PendingExpenses = ({ expenses, onAddExpensePayment }) => {
    ...
    return <div className={listStyles.surface}>{expenses.length ? expenses.map((expense) => {
        ...
        return <div className={`${listStyles.row} ${styles.pendingRow}`} key={expense.id}><button type="button" className={listStyles.icon} style={{ color: categoryColor, backgroundColor: `${categoryColor}22` }} onClick={() => router.push(`/expenses/details/${expense.id}`)} aria-label={t('Edit')}><i className={expense.category_icon || 'bi bi-receipt'} aria-hidden="true" /></button><button type="button" className={`${listStyles.rowContent} ${styles.pendingContent}`} onClick={() => router.push(`/expenses/details/${expense.id}`)}><span className={listStyles.primary}><i className={styles.categoryDot} style={{ backgroundColor: categoryColor }} />{expense.name}</span><span className={listStyles.meta}>{expense.formattedDueDate}</span></button><div className={styles.pendingActions}><span className={isPaid ? listStyles.badge : listStyles.pendingBadge}>{isPaid ? t('Paid') : t('Pending')}</span>{!isPaid && <ExpensePayment expense={expense} onAddExpensePayment={onAddExpensePayment} renderTrigger={(open) => <button type="button" className={listStyles.pay} onClick={open}>{t('Pay')}</button>} />}</div><button type="button" className={styles.progressContent} onClick={() => router.push(`/expenses/details/${expense.id}`)}>{expense.expense_amounts?.map((item) => {
