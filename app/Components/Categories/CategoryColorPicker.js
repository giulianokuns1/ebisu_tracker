...
export default function CategoryColorPicker({ color, onChange }) {
    ...
    return <div className={styles.picker} ref={containerRef}>
        ...
        {open && <div className={styles.popover} role="dialog" aria-label={t('Category color picker')}><HexColorPicker color={color} onChange={onChange} /><div className={styles.hexField}><span>#</span><HexColorInput color={color} onChange={onChange} prefixed={false} aria-label={t('Hex color')} /></div><div className={styles.presets}>{presets.map((preset) => <button key={preset} className={preset === color.toUpperCase() ? styles.selectedPreset : styles.preset} type="button" onClick={() => onChange(preset)} aria-label={`${t('Use')} ${preset}`}><span style={{ backgroundColor: preset }} /></button>)}</div></div>}
    </div>;
}
