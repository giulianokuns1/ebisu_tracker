    ...
    return (
        <div className={styles.workspace}>
            ...
            <section className={styles.listPanel}>
                ...
                {visibleExpenses.length ? <div className={styles.table} role="table"><div className={`${styles.tableRow} ${styles.tableHead}`} role="row"><span>{t('Due Date')}</span><span>{t('Description')}</span><span>{t('Category')}</span><span>{t('Amount')}</span><span>{t('Status')}</span><span>{t('Actions')}</span></div>{visibleExpenses.map((expense) => <button key={expense.id} type="button" className={styles.tableRow} onClick={() => router.push(`/expenses/details/${expense.id}`)}><span>{t(expense.formattedGridDueDate)}</span><span className={styles.description}><i className={expense.category_icon || 'bi bi-receipt'} style={{ color: expense.category_color || '#809297' }} aria-hidden="true" />{t(expense.name)}</span><span className={styles.categoryChip}><i className={styles.categoryDot} style={{ backgroundColor: expense.category_color || '#809297' }} />{t(expense.category_name || 'Other')}</span><span className={isExpensePaid(expense) ? styles.paidAmount : styles.pendingAmount}>{expense.amounts.map((amount) => <span key={amount.expense_amount_id} className={styles.multiAmount}>{amount.currency_symbol} {format(amount.amount)}</span>)}</span><span className={isExpensePaid(expense) ? styles.paidStatus : styles.pendingStatus}>{isExpensePaid(expense) ? t('Paid') : t('Pending')}</span><span className={styles.edit}><i className="bi bi-pencil" aria-hidden="true" /><span className="visually-hidden">{t('Edit')}</span></span></button>)}</div> : <div className={styles.emptyState}><i className="bi bi-receipt" aria-hidden="true" /><strong>{t('No expenses to show.')}</strong><span>{t('Add an expense or change the selected filters.')}</span></div>}
            </section>
        </div>
    );
};
