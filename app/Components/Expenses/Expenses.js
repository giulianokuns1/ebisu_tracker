        ...
        <div className={styles.workspace}>
            ...
            <section className={styles.overviewGrid}>
                ...
                <article className={`${styles.panel} ${styles.categoryPanel}`}>
                    <PanelTitle eyebrow={t('Expenses by Category')} title={`${data.totalEntries || 0} ${t('Transactions')}`} currencies={currencies} value={currencyId} onChange={setCurrencyId} includeAll />
                    <div className={styles.categoryContent}><div className={styles.categoryList}>{categories.slice(0, 5).map((category, index) => { const color = category.color || categoryColors[index]; const showAllCurrencies = currencyId === 'all'; return <div className={styles.categoryRow} key={category.id}><span className={styles.categoryIcon} style={{ color }}><i className={category.icon} aria-hidden="true" /></span><strong>{t(category.name)}</strong>{!showAllCurrencies && <div className={styles.categoryRail}><span style={{ background: color, width: `${total ? (category.amount / total) * 100 : 0}%` }} /></div>}<b className={showAllCurrencies ? styles.categoryAmounts : ''}>{category.currencyAmounts.map((item) => <span key={item.symbol}>{item.symbol} {formatCategoryAmount(item.amount)}</span>)}</b>{!showAllCurrencies && <small>{total ? ((category.amount / total) * 100).toFixed(1) : 0}%</small>}</div>; })}</div></div>
                </article>
            </section>
