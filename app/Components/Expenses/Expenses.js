...
const Expenses = ({ expenses, monthText, data }) => {
    ...
    const categorySummary = expenses.filter((expense) => !expense.is_credit_card_purchase).reduce((summary, expense) => {
        ...
        const currencyKey = String(expense.currency_id || expense.currency_symbol || 'unknown');
        if (!summary[key].amounts[currencyKey]) summary[key].amounts[currencyKey] = { amount: 0, symbol: expense.currency_symbol || '' };
        summary[key].amounts[currencyKey].amount += amount;
        summary[key].count += 1;
        return summary;
    }, {});
    const categories = Object.values(categorySummary).map((category) => ({
        ...category,
        amount: category.amounts[activeCurrencyId]?.amount || 0,
        currencyAmounts: Object.values(category.amounts).filter((item) => currencyId === 'all' || item.symbol === currency.symbol),
    })).filter((category) => currencyId === 'all' || category.amount > 0).sort((a, b) => {
        if (currencyId !== 'all') return b.amount - a.amount;
    ...
    const formatCategoryAmount = (amount) => new Intl.NumberFormat(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 }).format(Number(amount) || 0);
    const isPaid = (expense) => Boolean(expense.isFullPaid) || Number(expense.paymentTotal || 0) >= Number(expense.amount || 0);
    const groupedExpenses = Object.values(expenses.reduce((grouped, expense) => {
        if (!grouped[expense.id]) grouped[expense.id] = { ...expense, amounts: [] };
        grouped[expense.id].amounts.push(expense);
        return grouped;
    }, {}));
    const isExpensePaid = (expense) => expense.amounts.every(isPaid);
    const visibleExpenses = groupedExpenses.filter((expense) => status === 'all' || (status === 'paid' ? isExpensePaid(expense) : !isExpensePaid(expense)));
    const statusCount = (targetStatus) => groupedExpenses.filter((expense) => targetStatus === 'paid' ? isExpensePaid(expense) : !isExpensePaid(expense)).length;
    return (
        <div className={styles.workspace}>
            ...
            <section className={styles.overviewGrid}>
                ...
                <article className={`${styles.panel} ${styles.categoryPanel}`}>
                    <PanelTitle eyebrow={t('Expenses by Category')} title={t('Scheduled allocation')} currencies={currencies} value={currencyId} onChange={setCurrencyId} includeAll />
                    <div className={styles.categoryContent}><div className={styles.categoryList}>{categories.slice(0, 5).map((category, index) => { const color = category.color || categoryColors[index]; const showAllCurrencies = currencyId === 'all'; return <div className={styles.categoryRow} key={category.id}><span className={styles.categoryIcon} style={{ color }}><i className={category.icon} aria-hidden="true" /></span><strong>{t(category.name)}</strong>{!showAllCurrencies && <div className={styles.categoryRail}><span style={{ background: color, width: `${total ? (category.amount / total) * 100 : 0}%` }} /></div>}<b className={showAllCurrencies ? styles.categoryAmounts : ''}>{category.currencyAmounts.map((item) => <span key={item.symbol}>{item.symbol} {formatCategoryAmount(item.amount)}</span>)}</b>{!showAllCurrencies && <small>{total ? ((category.amount / total) * 100).toFixed(1) : 0}%</small>}</div>; })}</div><div className={styles.transactionCount}>{data.totalEntries || 0}<span>{t('Transactions')}</span></div></div>
                </article>
            </section>

            <section className={styles.listPanel}>
                <div className={styles.listHeader}><div><p>{t('Expenses List')}</p><span>{t('Showing')} {visibleExpenses.length} {t('of')} {groupedExpenses.length} {t('entries')}</span></div><div className={styles.tabs}>{[['all', t('All')], ['paid', t('Paid')], ['pending', t('Pending')]].map(([key, label]) => <button key={key} type="button" onClick={() => setStatus(key)} className={status === key ? styles.activeTab : ''}>{label}{key !== 'all' && <small>{statusCount(key)}</small>}</button>)}</div></div>
                {visibleExpenses.length ? <div className={styles.table} role="table"><div className={`${styles.tableRow} ${styles.tableHead}`} role="row"><span>{t('Due Date')}</span><span>{t('Description')}</span><span>{t('Category')}</span><span>{t('Amount')}</span><span>{t('Status')}</span><span>{t('Actions')}</span></div>{visibleExpenses.map((expense) => <button key={expense.id} type="button" className={styles.tableRow} onClick={() => router.push(`/expenses/details/${expense.id}`)}><span>{t(expense.formattedGridDueDate)}</span><span className={styles.description}><i className={expense.category_icon || 'bi bi-receipt'} style={{ color: expense.category_color || '#809297' }} aria-hidden="true" />{t(expense.name)}{expense.is_credit_card_purchase && expense.creditCardStatementName && <small className={styles.creditCardBadge}><i className="bi bi-credit-card" aria-hidden="true" />{t(expense.creditCardStatementName)}</small>}</span><span className={styles.categoryChip}><i className={styles.categoryDot} style={{ backgroundColor: expense.category_color || '#809297' }} />{t(expense.category_name || 'Other')}</span><span className={isExpensePaid(expense) ? styles.paidAmount : styles.pendingAmount}>{expense.amounts.map((amount) => <span key={amount.expense_amount_id} className={styles.multiAmount}>{amount.currency_symbol} {format(amount.amount)}</span>)}</span><span className={isExpensePaid(expense) ? styles.paidStatus : styles.pendingStatus}>{isExpensePaid(expense) ? t(expense.is_credit_card_purchase ? 'Paid by credit' : 'Paid') : t('Pending')}</span><span className={styles.edit}><i className="bi bi-pencil" aria-hidden="true" /><span className="visually-hidden">{t('Edit')}</span></span></button>)}</div> : <div className={styles.emptyState}><i className="bi bi-receipt" aria-hidden="true" /><strong>{t('No expenses to show.')}</strong><span>{t('Add an expense or change the selected filters.')}</span></div>}
            </section>
        </div>
    );
};
...
const PanelTitle = ({ eyebrow, title, currencies, value, onChange, includeAll = false }) => <header className={styles.panelTitle}><div><p>{eyebrow}</p><h2>{title}</h2></div>{currencies?.length > 1 && <select value={value} onChange={(event) => onChange(event.target.value)}>{includeAll && <option value="all">{`All ${currencies.length} currencies`}</option>}{currencies.map(([id, item]) => <option key={id} value={id}>{item.currency?.symbol} {item.currency?.name}</option>)}</select>}</header>;
